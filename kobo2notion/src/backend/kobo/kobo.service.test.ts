...
describe('KoboService', () => {
    ...
    `);
        ...
        for (const bookmark of mockBookmarks) {
            await db.run(
                ...
                [bookmark.volumeId, bookmark.highlight, bookmark.annotation, bookmark.createdOn, bookmark.type]
            );
        }
    ...
    describe('getBookmarks', () => {
        ...
        it('should throw an error if no content ID is found for the given title', async () => {
            await expect(service.getBookmarks('Nonexistent Book')).rejects.toThrow('No content ID found for title: Nonexistent Book');
        });
    });
});