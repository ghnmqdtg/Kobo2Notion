...
async function updateEnvFile(entries: { key: string; value: string }[]): Promise<void> {
  ...
  entries.forEach((entry) => {
    const matchingLineIndex = lines.findIndex((line) => line.match(new RegExp(entry.key)))
...
function createWindow(): void {
  ...
  // Bypass CORS for Notion API requests (needed because @notionhq/client v5
  // uses globalThis.fetch which in Electron's renderer enforces CORS)
