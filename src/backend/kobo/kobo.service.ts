...
export class KoboService {
  ...
  private async getBookmarkCountMap(): Promise<Map<string, number>> {
    ...
    const bookmarkCounts = await this.db.all<
      { volumeId: string; count: number }[]
    >(`
      SELECT
        CASE
  ...
  private mimeTypeToSource(mimeType: string): BookSource {
    ...
    if (mimeType === "application/epub+zip" || mimeType === "application/pdf")
      return "external";
    return "kobo-store";
  }
  ...
  async getBooks(): Promise<Book[]> {
    ...
    const [storeBooks, nonStoreBooks] = await Promise.all([
      this.db.all<(Book & { contentId: string; mimeType: string })[]>(
        storeQuery,
      ),
      this.db.all<(Book & { contentId: string; mimeType: string })[]>(
        nonStoreQuery,
    ...
    console.info(
      `Retrieved ${booksWithCounts.length} items (${storeBooks.length} store, ${nonStoreBooks.length} external)`,
    );
