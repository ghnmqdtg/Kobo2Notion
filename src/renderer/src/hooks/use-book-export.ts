...
interface UseBookExportProps {
  ...
  setExportedBooks: (books: Set<string> | ((prev: Set<string>) => Set<string>)) => void
...
interface UseBookExportReturn {
  ...
  handleCancel: () => void
  handleOverwriteConfirm: (selectedPageIds: string[], skippedBooks: string[]) => Promise<void>
...
export function useBookExport({
  ...
  const updateStates = (exporting: boolean, canceling: boolean, checking: boolean): void => {
  ...
  const startExport = async (skippedBooks: string[] = []): Promise<void> => {
    ...
    try {
      for (const bookTitle of remainingBooks) {
        ...
        setUploadedPages((prev) => [...prev, { pageId: parentPageId, bookTitle: book.bookTitle }])
  ...
  const handleOverwriteCancel = (): void => {
    setShowOverwriteDialog(false)
    setExistingPages([])
    setExportedBooks(new Set())
    updateStates(false, false, false)
