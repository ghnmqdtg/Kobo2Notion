...
export function BookmarksPreviewDialog({ open, onOpenChange, bookTitle }: BookmarksPreviewDialogProps) {
    ...
    const renderContent = () => {
        ...
        const hasAnyAnnotation = bookmarks.some(b => b.annotation && b.annotation.trim() !== "");
            ...
            <Table>
                <TableHeader>
                    <TableRow>
                        ...
                        <TableHead className={hasAnyAnnotation ? "w-3/5" : "w-full"}>Bookmark</TableHead>
                        {hasAnyAnnotation && (
                ...
                <TableBody>
                    {bookmarks.map((bookmark, index) => (
                        <TableRow key={index}>
                            ...
                            <TableCell className="select-text">{bookmark.highlight}</TableCell>
                            {hasAnyAnnotation && (
                                <TableCell className="select-text">{bookmark.annotation}</TableCell>
                            )}
                        </TableRow>
                    ))}
                </TableBody>
