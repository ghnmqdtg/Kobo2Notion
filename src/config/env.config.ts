...
const defaultConfig: EnvironmentConfig = {
  SQLITE_SOURCE:
    process.platform === 'darwin' ? '/Volumes/KOBOeReader/.kobo/KoboReader.sqlite' : '',
  NOTION_API_KEY: '',
...
const initConfig = (): EnvironmentConfig => {
  ...
  const provider = process.env.LLM_PROVIDER || legacyProvider || defaultConfig.LLM_PROVIDER

  const perProviderKeys: Pick<
    EnvironmentConfig,
    'LLM_API_KEY_GOOGLE' | 'LLM_API_KEY_OPENAI' | 'LLM_API_KEY_ANTHROPIC'
  > = {
    LLM_API_KEY_GOOGLE:
      process.env.LLM_API_KEY_GOOGLE ||
      (legacyApiKey && provider === 'google' ? legacyApiKey : '') ||
      defaultConfig.LLM_API_KEY_GOOGLE,
    LLM_API_KEY_OPENAI: process.env.LLM_API_KEY_OPENAI || defaultConfig.LLM_API_KEY_OPENAI,
    LLM_API_KEY_ANTHROPIC: process.env.LLM_API_KEY_ANTHROPIC || defaultConfig.LLM_API_KEY_ANTHROPIC
  }
  ...
  const config: EnvironmentConfig = {
    ...
    NOTION_DATA_SOURCE_ID: process.env.NOTION_DB || defaultConfig.NOTION_DATA_SOURCE_ID,
